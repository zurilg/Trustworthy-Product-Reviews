...
public class SearchValidationAspect {
    ...
    public void validateSearch(String search) {
        /* Our search validation logic will validate the following:
               1.) Search string is not null or empty
               2.) Search string length is between 1 and 100 characters
               3.) Search string does not contain more than 15 words
         */
        if (search != null) {
            String trimmedSearch = search.trim();
            if (trimmedSearch.isEmpty()) {
                throw new IllegalSearchException("Search cannot be empty.");
            }
